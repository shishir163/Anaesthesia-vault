  ...
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = (e) => {
      ...
      if(!db.objectStoreNames.contains('checklist')){
        // one record per syllabus section: { id, order, name, color, kcs:[...] }
...
const DB = {
  ...
  // ---- checklist (FRCA/FCPS syllabus, one record per domain) ----
  async putChecklist(domain){
    const store = await tx('checklist','readwrite');
    return new Promise((res,rej)=>{
      const r = store.put(domain);
      r.onsuccess = ()=>res(domain);
      r.onerror = ()=>rej(r.error);
    });
  },
  ...
  // ---- bulk import / export ----
  async exportAll(){
    const [notes, folders, tasks, slots, activity, checklist] = await Promise.all([
      DB.getAllNotes(), DB.getAllFolders(), DB.getAllTasks(), DB.getAllSlots(), DB.getAllActivity(), DB.getAllChecklist()
    ]);
    return { app:'anaesthesia-vault', version:1, exportedAt:new Date().toISOString(),
             notes, folders, tasks, slots, activity, checklist };
  },

  async importAll({notes=[], folders=[], tasks=[], slots=[], activity=[], checklist=[]}){
    const db = await openDB();
    return new Promise((res, rej) => {
      const t = db.transaction(['notes','folders','tasks','slots','activity','checklist'], 'readwrite');
      ...
      checklist.forEach(c => t.objectStore('checklist').put(c));
    });
  }
};
