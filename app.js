...
const state = {
  ...
  // ---- Phase 7: syllabus checklist ----
  checklist: [],     // array of section records {id,order,name,color,kcs:[]}
  clOpen: {},        // open domains  { domId:true }
  clOpenK: {},       // open key capabilities { kcId:true }
  clEdit: null,      // inline edit  { kind:'domain'|'kc'|'item', id, d, k, fresh }
  clTagFor: null,    // itemId whose tag menu is open
};
...
async function boot(){
  ...
  state.checklist = await DB.getAllChecklist();
...
function renderSidebar(){
  ...
  else if(state.mode === 'checklist') renderChecklistSidebar(body);
...
function switchMode(mode){
  state.mode = mode;
  const modes = { home:'#modeHomeBtn', notes:'#modeNotesBtn', planner:'#modePlannerBtn', revise:'#modeReviseBtn', checklist:'#modeChecklistBtn' };
  Object.entries(modes).forEach(([m,sel]) => { const b=$(sel); if(b) b.classList.toggle('active', mode===m); });
  ...
  $('#checklistContent').style.display = mode==='checklist' ? '' : 'none';
...
async function importBackup(file){
  ...
  const counts = ['notes','folders','tasks','slots','activity','checklist']
  ...
  try{
    await DB.importAll({
      ...
      slots: data.slots||[], activity: data.activity||[], checklist: data.checklist||[]
    });
    state.notes    = await DB.getAllNotes();
    state.folders  = await DB.getAllFolders();
    state.tasks    = await DB.getAllTasks();
...
function bindGlobalEvents(){
  ...
  $('#modeChecklistBtn').onclick = () => switchMode('checklist');
  $('#checklistBody').addEventListener('click', onChecklistClick);
  $('#checklistBody').addEventListener('keydown', onChecklistKey);
...
/* ============================================================
   Phase 7 — FRCA / FCPS syllabus checklist
   Domain -> subgroup -> item (tick, add, edit, delete).
   One record per domain in the 'checklist' store.
   ============================================================ */

const CL_TAGS = ['Must-Know','High-Yield','Hard Topic','Written','Viva','OSPE','Long Case','Short Case'];

/* Build a domain record from plain [groupTitle, [items...]] pairs,
   generating stable ids so ticks/edits persist. */
function clBuild(id, order, name, color, groups){
  return { id, order, name, color, kcs: groups.map((g, gi) => ({
    id: id+'-k'+gi, title: g[0],
    items: g[1].map((t, ii) => ({ id: id+'-k'+gi+'-i'+ii, text: t, tags: [], done: false }))
  })) };
}

const FRCA_SEED = [
  clBuild('pom', 10, 'Perioperative Medicine & Health Promotion', '#0EA5E9', [
    ['Preoperative assessment & optimisation', [
      'Structured preoperative assessment across ASA 1–4; history, examination and chart review',
      'Assessing the airway before surgery — clinical signs and relevant imaging',
      'Risk assessment, communicating risk and shared decision-making',
      'Functional capacity and fitness testing, including CPET',
      'Balancing early surgery against investigation, prehabilitation and optimisation',
    ]],
    ['Chronic disease in the surgical patient', [
      'Cardiovascular disease and perioperative cardiac risk',
      'Respiratory disease and preoperative optimisation',
      'Diabetes and endocrine disease in the perioperative period',
      'Renal and hepatic impairment',
      'Perioperative management of medicines, anticoagulants and antiplatelets',
      'Perioperative anaemia and coagulopathy',
    ]],
    ['Special considerations', [
      'Frailty, cognitive impairment and the older surgical patient',
      'Obesity and the bariatric patient',
      'Substance misuse and dependency',
      'Nutritional state — assessment and consequences of poor nutrition',
      'Effects of ethnicity and patient diversity on assessment',
    ]],
    ['Health promotion & the wider pathway', [
      'Smoking, alcohol and lifestyle optimisation before surgery',
      'Enhanced recovery principles',
      'Discharge planning as part of perioperative care',
    ]],
    ['Obstetric perioperative care', [
      'Planning anaesthetic care with the maternity team',
      'Recognising and managing acute obstetric emergencies',
    ]],
  ]),

  clBuild('ga', 20, 'General Anaesthesia', '#6366F1', [
    ['Applied basic sciences in practice', [
      'Anatomy, physiology, pharmacology, physics and measurement applied to anaesthesia',
      'Drug toxicity — causes, avoidance and the risks of additives',
      'Malignant hyperthermia — recognition and management',
      'Therapeutics in disease states — transplantation, malignancy, respiratory disease',
    ]],
    ['Conduct of anaesthesia', [
      'Total intravenous anaesthesia — models, monitoring and avoiding awareness',
      'Depth-of-anaesthesia monitoring and its interpretation',
      'Goal-directed therapy',
      'Patient positioning and prevention of position-related injury',
    ]],
    ['Extremes of age', [
      'Anaesthesia for neonates, children and the frail elderly',
      'Effect of age on drug handling',
      'Minimising postoperative cognitive dysfunction',
    ]],
    ['Complex airway, head & neck', [
      'Assessment and planning for the complex or threatened airway',
      'Videolaryngoscopy and fibreoptic intubation',
      'Failed intubation and front-of-neck access',
      'Tracheostomy — indications, care and the obstructed or misplaced tube',
      'Shared-airway and laser surgery; anaesthesia below the cords',
      'Head and neck cancer surgery; acute ENT emergencies',
    ]],
    ['Haemostasis & major haemorrhage', [
      'Intraoperative haemostasis and major blood loss',
      'Massive transfusion, rapid infusion and cell salvage',
      'Trauma haemorrhage — permissive hypotension and tranexamic acid',
    ]],
    ['Trauma, burns & poisoning', [
      'Anaesthesia for the multiply injured adult',
      'Coagulopathy, hypothermia and acidosis in trauma',
      'Reducing secondary brain injury after head injury',
      'Burns — pathophysiology and anaesthetic management',
      'Acute poisoning; drowning, electrical and crush injuries',
    ]],
    ['Neuroanaesthesia', [
      'Applied neuroanatomy and physiology; control of intracranial pressure',
      'Anaesthesia for intracranial, spinal and neuroradiology procedures',
      'Awake craniotomy and neurophysiological monitoring',
      'Venous air embolism — recognition and management',
    ]],
    ['Cardiac & thoracic anaesthesia', [
      'Invasive and advanced haemodynamic monitoring; echo and TOE',
      'Cardiovascular physiology and pharmacology for cardiac surgery',
      'Cardiopulmonary bypass — principles and consequences',
      'Off-pump surgery, TAVI and electrophysiology procedures',
      'Pacing and mechanical circulatory support',
      'One-lung ventilation for thoracic surgery',
    ]],
    ['Major cavity, vascular & robotic surgery', [
      'Major abdominal, hepatobiliary and oesophageal resection',
      'Open and endovascular aortic surgery; carotid surgery',
      'Aortic cross-clamping and organ protection',
      'Robotic and laparoscopic surgery — physiological effects',
    ]],
    ['Obstetric anaesthesia', [
      'Physiological changes of pregnancy and their anaesthetic implications',
      'Anaesthesia and analgesia for labour and delivery',
      'Pre-eclampsia, eclampsia and obstetric haemorrhage',
      'Amniotic fluid embolism, sepsis and abnormal placentation',
      'Complications of neuraxial block; post-dural puncture headache',
      'The pregnant patient for non-obstetric surgery',
    ]],
    ['Paediatric & neonatal anaesthesia', [
      'Developmental anatomy and physiology relevant to anaesthesia',
      'Anaesthesia for common elective and emergency paediatric surgery',
      'Fluid balance and blood products in children',
      'The child with complex or congenital problems',
      'Principles of neonatal anaesthesia; thermoregulation and prematurity',
      'Consent, restraint and Gillick competence',
    ]],
    ['Non-theatre anaesthesia', [
      'Anaesthesia in radiology, MRI, endoscopy, ECT and the emergency department',
      'Environment-specific safety, especially MRI',
      'Safe recovery outside the operating theatre',
    ]],
  ]),

  clBuild('ra', 30, 'Regional Anaesthesia', '#0D9488', [
    ['Foundations', [
      'Principles of ultrasound, obtaining an optimal image and recognising artefacts',
      'Pharmacology of local anaesthetics and adjuvants',
      'Local anaesthetic systemic toxicity — prevention and management',
      'Safety and quality; preventing wrong-side block',
      'Involving the patient — indications, contraindications and complications',
    ]],
    ['Upper limb & neuraxial', [
      'Ultrasound-guided brachial plexus and upper limb blocks',
      'Central neuraxial blockade',
    ]],
    ['Trunk & fascial plane blocks', [
      'Chest wall blocks and analgesia for rib fractures — erector spinae, paravertebral, serratus',
      'Abdominal wall blocks — rectus sheath, TAP, quadratus lumborum',
      'Blocks for lower abdominal and urological surgery — inguinal, penile, caudal, pudendal',
    ]],
    ['Lower limb & specialist blocks', [
      'Lower limb blocks and their complications, including fascia iliaca and PENG',
      'Cervical plexus blocks for awake carotid surgery',
      'Ophthalmic blocks; the oculocardiac reflex and control of intraocular pressure',
      'Regional anaesthesia in children',
    ]],
  ]),

  clBuild('rt', 40, 'Resuscitation & Transfer', '#DC2626', [
    ['Leading resuscitation', [
      'Leading the resuscitation team from arrest to handover',
      'Recognising and stabilising the deteriorating perioperative patient',
      'The patient with an unstable cervical spine',
    ]],
    ['Peri-arrest & advanced life support', [
      'Peri-arrest arrhythmias — recognition and interpretation',
      'Drugs for arrhythmias; cardioversion, defibrillation and pacing',
      'Anaphylaxis; thrombolysis; post-arrest care and targeted temperature',
      'Special situations — prone arrest, open-chest compressions, thoracotomy',
    ]],
    ['Trauma resuscitation', [
      'Role of the anaesthetist in the trauma team; a structured approach',
      'Chest, abdominal, head, neck, spinal, pelvic and limb trauma',
      'Triage and scoring; the metabolic response to trauma',
    ]],
    ['Paediatric & neonatal resuscitation', [
      'Resuscitation of children and neonates and how it differs from adults',
      'The premature delivery',
    ]],
    ['Acute neurological emergencies', [
      'Acute spinal cord injury and traumatic brain injury',
      'Neurocritical care — ICP control, cerebral protection and monitoring',
      'Control of status epilepticus',
    ]],
    ['Transfer medicine', [
      'Safe inter-hospital transfer of adults and children, including time-critical transfers',
      'Stabilisation, packaging, monitoring and equipment for transfer',
      'Hazards of transfer — acceleration, noise and the transfer environment',
      'Transfer of specific groups — brain injury, burns, children and pregnancy',
    ]],
    ['Decision-making & ethics', [
      'DNACPR discussions with patients and families',
      'Ethics of the multiply injured patient, brainstem death and organ donation',
    ]],
  ]),

  clBuild('ps', 50, 'Procedural Sedation', '#7C3AED', [
    ['Sedation technique', [
      'Levels of sedation — minimal, conscious and deep',
      'Pharmacology of sedatives, infusions and target-controlled infusion',
      'Alternative routes — intranasal and rectal',
      'Sedation for specific procedures — ophthalmic, dental, neuro',
    ]],
    ['Safe delivery', [
      'Sedation scoring across adults, children and special-needs patients',
      'Standards for equipment, staffing and monitoring',
      'Discharge criteria after sedation',
      'Risks of sedation outside theatre and how to mitigate them',
    ]],
    ['Knowing the limits', [
      'Recognising when sedation is unsafe or inappropriate',
      'Planning a safe alternative when sedation cannot be delivered',
    ]],
  ]),

  clBuild('pa', 60, 'Pain Medicine', '#DB2777', [
    ['Foundations of pain', [
      'Mechanisms of pain — nociceptive, neuropathic and nociplastic',
      'Peripheral and central sensitisation',
      'Anatomy of ascending and descending pain pathways',
      'Influence of genetics, culture, mental health and early-life experience',
    ]],
    ['Acute pain', [
      'Assessment of pain and use of assessment tools',
      'Managing postoperative pain, including complex analgesic needs',
      'Adjuvant analgesics, regional techniques and wound infiltration',
      'Acute pain in special groups — children, elderly, cognitively impaired, opioid-tolerant, critically ill',
      'Acute-on-chronic pain and the patient on long-term opioids',
    ]],
    ['Chronic & cancer pain', [
      'Assessment and management of chronic primary and secondary pain',
      'Cancer pain',
      'Advanced pharmacology — opioids, antidepressants, anticonvulsants, antiarrhythmics',
      'Interventional procedures, TENS and other therapies',
      'Barriers to effective pain management',
      'Persistent post-surgical pain — risk factors and prevention',
    ]],
  ]),

  clBuild('icm', 70, 'Intensive Care Medicine', '#EA580C', [
    ['Admission, triage & the deteriorating patient', [
      'When intensive care helps and where its limits lie',
      'Admission and discharge decisions; ceilings of care and escalation',
      'Early-warning scores, outreach and detecting deterioration early',
    ]],
    ['Respiratory support', [
      'Respiratory failure — causes and oxygen delivery versus demand',
      'Invasive and non-invasive ventilation; choosing the mode',
      'Consequences of positive-pressure ventilation',
      'ARDS — lung-protective ventilation and proning',
      'Rescue therapies — nitric oxide, heliox and ECMO',
      'Severe asthma and COPD; weaning and failed extubation',
      'Indications for postoperative ventilation',
    ]],
    ['Cardiovascular support', [
      'Shock and the low- and high-output states',
      'Vasopressors and inotropes — rational choice',
      'Haemodynamic monitoring and fluid responsiveness',
      'The ischaemic, failing or valvular heart in critical illness',
    ]],
    ['Neurocritical care', [
      'Raised intracranial pressure and traumatic brain injury; protecting cerebral perfusion',
      'Seizures and status; delirium and the unconscious patient',
      'Spinal cord injury; neuromuscular disease such as myasthenia and Guillain-Barré',
      'Brainstem death — diagnosis and testing',
    ]],
    ['Kidney, fluids & metabolism', [
    ...
    ['Liver, gut & nutrition', [
      ...
      'Gastrointestinal failure and pancreatitis',
      'Nutrition — enteral versus parenteral, refeeding and overfeeding',
    ]],
    ['Sepsis & infection', [
      'Recognising and managing sepsis',
      'Antimicrobials — choice, resistance and stewardship',
      'Infection control and preventing surgical-site infection',
    ]],
    ['Endocrine & haematology', [
      'Diabetic, thyroid and adrenal emergencies',
      'Anaemia, thrombocytopenia and coagulopathy; transfusion',
      'Blood-borne viruses in the unit',
    ]],
    ['The critically ill child', [
      'Recognising the sick child',
      'Paediatric emergencies — sepsis, trauma, seizures, dehydration and DKA',
    ]],
    ['Humanity in critical care', [
      'Holistic care of the patient and family',
      'End-of-life care, withdrawal and palliation',
      'Organ donation and physiological support of the donor',
    ]],
  ]),
];
...
async function seedChecklist(){
  ...
  // remove the earlier prototype ICM seed (uppercase id) if it exists
  if(state.checklist.some(d => d.id === 'ICM')){
    await DB.deleteChecklist('ICM').catch(()=>{});
    state.checklist = state.checklist.filter(d => d.id !== 'ICM');
  }
  const cur = await DB.getSetting('checklistSeedVersion', 0);
  if(cur >= SEED_VERSION) return;
  for(const dom of FRCA_SEED){
    if(!state.checklist.some(d => d.id === dom.id)){
      const rec = JSON.parse(JSON.stringify(dom));
      await DB.putChecklist(rec);
      state.checklist.push(rec);
    }
  }
  await DB.setSetting('checklistSeedVersion', SEED_VERSION);
}

/* ---- lookups & stats ---- */
function clSortedDomains(){
  return [...state.checklist].sort((a,b) => (a.order||999)-(b.order||999) || (a.name||'').localeCompare(b.name||''));
}
function clFindDomain(id){ return state.checklist.find(d => d.id === id); }
function clKc(dom, kid){ return dom && (dom.kcs||[]).find(x => x.id === kid); }
function clItem(dom, kid, iid){ const kc = clKc(dom, kid); return kc && (kc.items||[]).find(x => x.id === iid); }
function clDomStats(d){ let t=0,x=0; (d.kcs||[]).forEach(k => (k.items||[]).forEach(i => { t++; if(i.done) x++; })); return {t,x,pct:t?Math.round(x/t*100):0}; }
function clKcStats(k){ const t=(k.items||[]).length, x=(k.items||[]).filter(i=>i.done).length; return {t,x,pct:t?Math.round(x/t*100):0}; }
function clIsEditing(kind,id){ return state.clEdit && state.clEdit.kind===kind && state.clEdit.id===id; }
function clPersist(dom){ return DB.putChecklist(dom).catch(()=>{}); }
function clLocate(kind,id){
  for(const dom of state.checklist){
    if(kind==='domain' && dom.id===id) return {dom, obj:dom};
    for(const kc of (dom.kcs||[])){
      if(kind==='kc' && kc.id===id) return {dom, obj:kc};
      for(const it of (kc.items||[])){ if(kind==='item' && it.id===id) return {dom, obj:it}; }
    }
  }
  return null;
}
...
/* ---- sidebar ---- */
function renderChecklistSidebar(body){
  const doms = clSortedDomains();
  let all=0, done=0; doms.forEach(d => { const s=clDomStats(d); all+=s.t; done+=s.x; });
  const pct = all ? Math.round(done/all*100) : 0;
  body.innerHTML =
    `<div style="padding:8px 6px 12px;font-family:var(--font-mono);font-size:12px;color:var(--text-dim)">Overall · ${done}/${all} · ${pct}%</div>` +
    doms.map(d => { const s=clDomStats(d);
      return `<button class="side-btn" data-cljump="${d.id}"><span>${escapeHtml(d.name)}</span><span class="count">${s.x}/${s.t}</span></button>`;
    }).join('');
  $$('[data-cljump]', body).forEach(b => b.onclick = () => {
    state.clOpen[b.dataset.cljump] = true;
    renderChecklist();
    const el = document.getElementById('cld-' + b.dataset.cljump);
...
/* ---- inline edit row ---- */
function clEditRow(curValue){
  return `<div class="cl-editwrap">
    <input id="clEditInput" class="cl-editinput" value="${escapeHtml(curValue)}" placeholder="Type, then press Enter">
    <button class="cl-iconbtn" data-cl="editSave" title="Save">${icon('check',16)}</button>
    <button class="cl-iconbtn" data-cl="editCancel" title="Cancel">${icon('x',16)}</button>
  </div>`;
}
...
function renderChecklist(){
  ...
  const sub = $('#checklistSub'); if(sub) sub.textContent = 'Tick as you finish. Add, edit or delete anything — it saves offline on this device.';
  const ov = $('#clOverall');
  if(ov) ov.innerHTML =
    `<div class="cl-ohead">
       <div><div class="cl-olabel">Overall coverage</div><div class="cl-opct">${pct}<span>%</span></div></div>
       <div class="cl-ocount">${done} of ${all} items done</div>
     </div>
     <div class="cl-bar" style="margin:12px 0 0;height:9px"><i style="width:${pct}%;background:var(--ecg)"></i></div>`;

  const html = doms.map(dom => {
    const s = clDomStats(dom);
    const open = !!state.clOpen[dom.id];
    const head =
      `<div class="cl-dhead" id="cld-${dom.id}" style="border-left-color:${dom.color}">
        <button class="cl-caret ${open?'open':''}" data-cl="togD" data-d="${dom.id}">${icon('chevR',18)}</button>
        ${ clIsEditing('domain', dom.id) ? clEditRow(dom.name)
          : `<div class="cl-dtitle">${escapeHtml(dom.name)}</div>
             <span class="cl-badge" style="background:${dom.color}22;color:${dom.color}">${s.x}/${s.t} · ${s.pct}%</span>
             <button class="cl-iconbtn" data-cl="edit" data-kind="domain" data-d="${dom.id}" title="Rename">${icon('pencil',15)}</button>
             <button class="cl-iconbtn danger" data-cl="del" data-kind="domain" data-d="${dom.id}" title="Delete domain">${icon('trash',15)}</button>` }
      </div>`;
    if(!open) return `<div class="cl-domain">${head}</div>`;

    const kcs = (dom.kcs||[]).map(kc => {
      const ks = clKcStats(kc);
      const kopen = !!state.clOpenK[kc.id];
      const khead =
        `<div class="cl-khead">
          <button class="cl-caret ${kopen?'open':''}" data-cl="togK" data-k="${kc.id}">${icon('chevR',15)}</button>
          ${ clIsEditing('kc', kc.id) ? clEditRow(kc.title)
            : `<div class="cl-ktitle">${escapeHtml(kc.title)}</div>
               <span class="cl-kcount">${ks.x}/${ks.t}</span>
               <button class="cl-iconbtn" data-cl="edit" data-kind="kc" data-d="${dom.id}" data-k="${kc.id}" title="Rename">${icon('pencil',13)}</button>
               <button class="cl-iconbtn danger" data-cl="del" data-kind="kc" data-d="${dom.id}" data-k="${kc.id}" title="Delete">${icon('trash',13)}</button>` }
        </div>
        <div class="cl-bar"><i style="width:${ks.pct}%;background:${dom.color}"></i></div>`;
      if(!kopen) return `<div class="cl-kc">${khead}</div>`;

      const items = (kc.items||[]).map(it => {
        if(clIsEditing('item', it.id)) return `<div class="cl-item"><div class="cl-body">${clEditRow(it.text)}</div></div>`;
        const chips = (it.tags||[]).map(t =>
          `<span class="badge ${badgeClass(t)}" data-cl="tagPick" data-d="${dom.id}" data-k="${kc.id}" data-i="${it.id}" data-tag="${t}" style="cursor:pointer" title="Remove ${t}">${t}</span>`).join('');
        const menu = state.clTagFor===it.id
          ? `<div class="cl-tagmenu">${CL_TAGS.map(t => { const on=(it.tags||[]).includes(t);
              return `<button class="exam-tag-btn ${on?'on':''}" data-tag="${t}" data-cl="tagPick" data-d="${dom.id}" data-k="${kc.id}" data-i="${it.id}">${t}</button>`; }).join('')}</div>`
          : '';
        return `<div class="cl-item">
          <button class="cl-check ${it.done?'done':''}" data-cl="tog" data-d="${dom.id}" data-k="${kc.id}" data-i="${it.id}" ${it.done?`style="background:${dom.color};border-color:transparent"`:''} aria-label="Toggle done">${icon('check',13)}</button>
          <div class="cl-body">
            <div class="cl-text ${it.done?'done':''}">${escapeHtml(it.text)}</div>
            <div class="cl-tags">${chips}<button class="cl-tagbtn" data-cl="tagMenu" data-i="${it.id}">${icon('plus',11)} tag</button>${menu}</div>
          </div>
          <button class="cl-iconbtn" data-cl="edit" data-kind="item" data-d="${dom.id}" data-k="${kc.id}" data-i="${it.id}" title="Edit">${icon('pencil',13)}</button>
          <button class="cl-iconbtn danger" data-cl="del" data-kind="item" data-d="${dom.id}" data-k="${kc.id}" data-i="${it.id}" title="Delete">${icon('trash',13)}</button>
        </div>`;
      }).join('');

      return `<div class="cl-kc">${khead}<div class="cl-items">${items}
        <button class="cl-add item" data-cl="addItem" data-d="${dom.id}" data-k="${kc.id}" style="color:${dom.color}">${icon('plus',14)} add item</button>
      </div></div>`;
    }).join('');

    return `<div class="cl-domain">${head}<div class="cl-dbody">${kcs}
      <button class="cl-add" data-cl="addKC" data-d="${dom.id}">${icon('plus',15)} add subgroup</button>
    </div></div>`;
  }).join('');

  const bodyEl = $('#checklistBody');
  if(bodyEl){
    bodyEl.innerHTML = html + `<div class="cl-footer"><button class="cl-add" data-cl="addDomain">${icon('plus',15)} Add section</button></div>`;
    if(state.clEdit){ const inp = document.getElementById('clEditInput'); if(inp){ inp.focus(); const v=inp.value; inp.value=''; inp.value=v; } }
  }
}

/* ---- event handling (delegated on #checklistBody) ---- */
async function onChecklistClick(e){
  const el = e.target.closest('[data-cl]'); if(!el) return;
  const a = el.dataset.cl, d = el.dataset.d, k = el.dataset.k, i = el.dataset.i;

  if(a==='togD'){ state.clOpen[d] = !state.clOpen[d]; return renderChecklist(); }
  if(a==='togK'){ state.clOpenK[k] = !state.clOpenK[k]; return renderChecklist(); }
  if(a==='editSave'){ return clEditSave(); }
  if(a==='editCancel'){ return clEditCancel(); }
  if(a==='addDomain'){
    const id = uid();
    const rec = { id, order:900, name:'', color:'#6366F1', kcs:[] };
    state.checklist.push(rec); state.clOpen[id] = true;
    state.clEdit = { kind:'domain', id, fresh:true };
    return renderChecklist();
  }

  const dom = clFindDomain(d); if(!dom) return;

  if(a==='tog'){ const it = clItem(dom,k,i); if(it){ it.done = !it.done; renderChecklist(); clPersist(dom); } return; }
  if(a==='tagMenu'){ state.clTagFor = (state.clTagFor===i ? null : i); return renderChecklist(); }
  if(a==='tagPick'){
    const it = clItem(dom,k,i); if(!it) return;
    it.tags = it.tags || [];
    const ix = it.tags.indexOf(el.dataset.tag);
    if(ix>=0) it.tags.splice(ix,1); else it.tags.push(el.dataset.tag);
    renderChecklist(); clPersist(dom); return;
  }
  if(a==='addItem'){
    const kc = clKc(dom,k); if(!kc) return;
    const it = { id:uid(), text:'', tags:[], done:false };
    kc.items.push(it); state.clOpenK[k] = true;
    state.clEdit = { kind:'item', id:it.id, d, k, fresh:true };
    return renderChecklist();
  }
  if(a==='addKC'){
    const kc = { id:uid(), title:'', items:[] };
    dom.kcs.push(kc); state.clOpen[d] = true;
    state.clEdit = { kind:'kc', id:kc.id, d, fresh:true };
    return renderChecklist();
  }
  if(a==='edit'){
    state.clEdit = { kind:el.dataset.kind, id:(el.dataset.kind==='domain'?d : el.dataset.kind==='kc'?k : i), d, k, fresh:false };
    return renderChecklist();
  }
  if(a==='del'){ return clDelete(el.dataset.kind, d, k, i); }
}

function onChecklistKey(e){
  if(e.target && e.target.id==='clEditInput'){
    if(e.key==='Enter'){ e.preventDefault(); clEditSave(); }
    else if(e.key==='Escape'){ e.preventDefault(); clEditCancel(); }
  }
}

async function clEditSave(){
  const ed = state.clEdit; if(!ed){ return; }
  const inp = document.getElementById('clEditInput');
  const v = inp ? inp.value.trim() : '';
  const loc = clLocate(ed.kind, ed.id);
  if(loc){
    if(ed.kind==='domain') loc.obj.name = v || 'Untitled section';
    else if(ed.kind==='kc') loc.obj.title = v || 'Untitled subgroup';
    else loc.obj.text = v || 'New item';
    await clPersist(loc.dom);
  }
  state.clEdit = null; renderChecklist();
}

async function clEditCancel(){
  const ed = state.clEdit; state.clEdit = null;
  if(ed && ed.fresh){ await clRemoveSilent(ed.kind, ed.d, ed.k, ed.id); }
  renderChecklist();
}

async function clRemoveSilent(kind, d, k, id){
  if(kind==='domain'){ state.checklist = state.checklist.filter(x => x.id!==id); await DB.deleteChecklist(id).catch(()=>{}); return; }
  const dom = clFindDomain(d); if(!dom) return;
  if(kind==='kc'){ dom.kcs = dom.kcs.filter(x => x.id!==id); await clPersist(dom); return; }
  if(kind==='item'){ const kc = clKc(dom,k); if(kc){ kc.items = kc.items.filter(x => x.id!==id); await clPersist(dom); } }
}

async function clDelete(kind, d, k, i){
  const dom = clFindDomain(d); if(!dom) return;
  if(kind==='domain'){
    if(!confirm(`Delete the entire "${dom.name}" section and everything in it?`)) return;
    state.checklist = state.checklist.filter(x => x.id!==d);
    await DB.deleteChecklist(d).catch(()=>{});
    renderChecklist(); return;
  }
  if(kind==='kc'){
    const kc = clKc(dom,k); if(!kc) return;
    if((kc.items||[]).length && !confirm(`Delete "${kc.title}" and its ${kc.items.length} item(s)?`)) return;
    dom.kcs = dom.kcs.filter(x => x.id!==k);
    renderChecklist(); clPersist(dom); return;
  }
  if(kind==='item'){
    const kc = clKc(dom,k); if(!kc) return;
    kc.items = kc.items.filter(x => x.id!==i);
    renderChecklist(); clPersist(dom);
  }
}
